...
// Orbiting Tech Radar — Responsive for mobile/tablet/desktop
// - Uses CSS variables per breakpoint for ring radii
// - Smaller icons + hide labels on phones, show labels from md+
// - Keeps cyan-400 theme and no extra vertical space

const Icons = {
  HTML5: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" className="w-5 h-5 md:w-6 md:h-6">
  ...
  JavaScript: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" className="w-5 h-5 md:w-6 md:h-6">
  ...
  React: (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className="w-5 h-5 md:w-6 md:h-6">
  ...
  Next: (
    <svg viewBox="0 0 15 15" className="w-5 h-5 md:w-6 md:h-6" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
function Badge({ children, label }) {
  return (
    <div className="relative rounded-xl border border-cyan-400/40 bg-cyan-400/5 backdrop-blur-sm p-1.5 md:p-2 shadow-[0_0_0_1px_rgba(0,255,255,0.15)_inset]">
      <div className="h-7 w-7 md:h-9 md:w-9 flex items-center justify-center text-white">{children}</div>
      {label ? (
        <span className="mt-1 hidden md:block text-[10px] text-center font-medium text-cyan-200">{label}</span>
      ) : null}
    </div>
  );
}

export default function TechRadarMBL() {
  // Per-orbit icon sets
  const outer = [Icons.HTML5, Icons.CSS3, Icons.JavaScript, Icons.React, Icons.Next, Icons.Tailwind];
  const middle = [Icons.Node, Icons.Express, Icons.MongoDB];
  const inner = [Icons.Git, Icons.GitHub, Icons.Figma];

  return (
    <div className="relative mx-auto aspect-square w-full max-w-[92vw] sm:max-w-[520px] md:max-w-[620px] ">
      ...
      <div className="relative h-full w-full">
        ...
        {/* Four rings sized with % so they scale with container */}
        {[20, 40, 60, 80].map((p, i) => (
          <div
            key={i}
        ...
        <OrbitRing varName="--rv-outer" duration={40} className="ring-outer">
          {outer.map((icon, i) => (
            <OrbitAngle key={i} deg={(360 / outer.length) * i} rVarName="--rv-outer">
              <Badge>{icon}</Badge>
            </OrbitAngle>
          ))}
        </OrbitRing>

        <OrbitRing varName="--rv-mid" duration={30} reverse className="ring-mid">
          {middle.map((icon, i) => (
            <OrbitAngle key={i} deg={(360 / middle.length) * i} rVarName="--rv-mid">
              <Badge>{icon}</Badge>
            </OrbitAngle>
          ))}
        </OrbitRing>

        <OrbitRing varName="--rv-inner" duration={22} className="ring-inner">
          {inner.map((icon, i) => (
            <OrbitAngle key={i} deg={(360 / inner.length) * i} rVarName="--rv-inner">
              <Badge>{icon}</Badge>
            </OrbitAngle>
          ))}
        </OrbitRing>
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="rounded-2xl border border-cyan-400/50 bg-cyan-400/10 px-3 py-1.5 text-[10px] md:text-xs text-cyan-100">Stack</div>
        </div>
      </div>

      {/* Responsive variables and keyframes */}
      <style>{`
        /* Default (desktop) ring radii */
        .ring-outer { --rv-outer: 33vmin; }
        .ring-mid   { --rv-mid:   25vmin; }
        .ring-inner { --rv-inner: 17vmin; }

        /* Tablet */
        @media (max-width: 1024px) {
          .ring-outer { --rv-outer: 28vmin; }
          .ring-mid   { --rv-mid:   21vmin; }
          .ring-inner { --rv-inner: 14vmin; }
        }
        /* Mobile */
        @media (max-width: 640px) {
          .ring-outer { --rv-outer: 22vmin; }
          .ring-mid   { --rv-mid:   16vmin; }
          .ring-inner { --rv-inner: 11vmin; }
        }
...
function OrbitRing({ children, varName = '--rv', duration = 30, reverse = false, className = '' }) {
  // Uses a CSS var for radius so we can override per breakpoint via media queries
  return (
    <div className={`absolute left-1/2 top-1/2 h-0 w-0 ${className}`} style={{ transform: 'translate(-50%, -50%)' }}>
      <div
        className="relative"
        style={{
          width: 'calc(var(' + varName + ') * 2)',
          height: 'calc(var(' + varName + ') * 2)',
          marginLeft: 'calc(var(' + varName + ') * -1)',
          marginTop: 'calc(var(' + varName + ') * -1)',
...
function OrbitAngle({ children, deg = 0, rVarName = '--rv' }) {
  return (
    <div
      className="absolute"
      style={{
        left: '50%',
        top: '50%',
        transform: `rotate(${deg}deg) translateY(calc(var(${rVarName}) * -1)) rotate(-${deg}deg)`,
      }}
    >
